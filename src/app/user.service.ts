...
signup(email:string,password:string)
{
  this.afAuth.auth.createUserWithEmailAndPassword(email,password).then(
    value => {
      this.toaster.success('Registered Successfully!...','Register');
    ...
    this.toaster.warning('This email ID already used by someone else!..','Error');
    this.router.navigate(['sign-up']);
    return;
    } 
    this.toaster.warning('something went wrong!..','Error');
    this.router.navigate(['sign-up']);
   })
}
updatetable(email:string)
{
  ...
  data['status']='offline';
  this.firestore.collection('users').doc(this.afAuth.auth.currentUser.uid).set(data);
}
...
login(email:string,password:string)
{
  ...
  this.afAuth.auth.signInWithEmailAndPassword(email,password).then(
    ...
    if (errorCode === 'auth/user-not-found') {
      // Call function X to sign user in instead
    this.toaster.warning('Account does not exist, Please Register!..','Error');
    this.router.navigate(['sign-up']);
    return;
    } 
    this.toaster.warning('something went wrong!..','Error');
...
Logout()
{
  ...
  this.toaster.success('You are logged out!..','Logout');
